...
export async function requireUser(req) {
  ...
  if (error || !data?.user) {
    const env = getSupabaseEnvStatus();
    const activeUrl = process.env[env.details?.urlKey] || 'UNKNOWN';

    console.error('[requireUser] getUser failed:', error?.message || 'no user', {
      tokenSnippet: token.slice(0, 10) + '...',
      projectUrl: activeUrl,
      usedKey: env.details?.roleKey,
      keySnippet: (process.env[env.details?.roleKey] || '').slice(0, 6) + '...'
    });
    return {
      error:
        error?.message ||
